...
class TripleImageDataset(data.Dataset):
    def __init__(self, video_root, video_list, rectify_label=None, transform=None):
        ...
        self.imgs_first, self.imgs_second, self.imgs_third, self.index = load_imgs_tsn(video_root, video_list, rectify_label)
        self.transform = transform
...
def load_imgs_tsn(video_root, video_list, rectify_label):
    ...
    with open(video_list, 'r') as imf:
        ...
        for id, line in enumerate(imf):
            ...
            # print(type(video_label[0]))
            label = rectify_label[int(video_label[1])]  # label of video
            ...
            index.append(np.ones(img_count) * id)  # id: 1 : 379
...
def load_imgs_tenfold_totalframe(video_root, video_list, rectify_label, fold, run_type):
    ...
    if run_type == 'train':
        ...
        fold_.remove(fold)  # [0,2,3,4,5,6,7,8,9, 10] -> [2,3,4,5,6,7,8,9,10]

        for i in fold_:
            fold_str = str(i) + '-fold'  # 0-fold
            for index, item in enumerate(
                    imf):  # 1, '0-fold\t31\n' in {[1, '0-fold\t31\n'], [0, 'S037/006 Happy\n'], ...}
                if fold_str in item:  # 0-fold in '0-fold\t31\n'
                    for j in range(index + 1, index + int(item.split()[1]) + 1):  # (1 + 0, 1 + 31 + 0 )
