...
class ResNet_AT(nn.Module):
    def __init__(self, block, layers, num_classes=1000, end2end=True, at_type=''):
        ...
        self.pred_fc1 = nn.Linear(512, 4)
        self.pred_fc2 = nn.Linear(1024, 4)
    ...
    def forward(self, x='', phrase='train', AT_level='first_level',vectors='',vm='',alphas_from1='',index_matrix=''):
        ...
        if phrase == 'train':
            ...
            for i in range(num_pair):
                ...
                f = f.squeeze(3).squeeze(2)  # f[0, 512, 0, 0] ---> f[0, 512]
        ...
        if phrase == 'eval':
            ...
            if AT_level == 'second_level':
                ...
                ''' alpha * beta '''
                weight_catefc = vs_cate.mul(alphas_from1)  # [21570,512] * [21570,0] --->[21570,512]
                alpha_beta = alphas_from1.mul(betas)
                sum_alphabetas = index_matrix.mm(alpha_beta)  # [380,21570] * [21570,0] -> [380,0]
